...
GenericValue CodeGenContext::runCode() {
    ...
    std::string err_str;
    ExecutionEngine *ee = EngineBuilder(llvm::make_unique<Module>("main", Context)).setErrorStr(&err_str).create();
...
Value* NInteger::codeGen(CodeGenContext& context)
{
    ...
    return ConstantInt::get(Type::getInt64Ty(Context), value, true);
}
...
{
    vector<Type*> argTypesVec;
    VariableList::const_iterator it;
    for (it = arguments.begin(); it != arguments.end(); it++) {
        argTypesVec.push_back(typeOf((**it).type));
    }
